    ...
    <el-card class="operation-area">
      ...
      <div class="operation-content" v-if="showPreview">
        <div class="image-container" v-loading="imageLoading">
          ...
          <div
            v-else-if="isNeedling"
            class="needling-area"
            ref="needlingArea"
            @click="handleNeedleClick"
          >
            <img :src="croppedImage" alt="裁剪后的图像" />
            <div
              v-for="(point, index) in needlePoints"
              :key="index"
              class="needle-point"
        ...
        <div class="result-images">
          <div class="result-image-item">
            ...
            <div class="result-image-container">
              <img
                :src="predictionResult.needledImageUrl"
                alt="布针图"
                class="result-image"
          ...
          <div class="result-image-item">
            ...
            <div class="result-image-container">
              <img
                :src="predictionResult.resultUrl"
                alt="预测结果"
                class="result-image"
                v-loading="predicting"
              />
...
const rules = {
  psa: [{ required: true, message: "请输入PSA值", trigger: "blur" }],
};
...
const getSequences = async (patientId) => {
  try {
    const token = localStorage.getItem("access_token");
    const response = await axios.get(
      `http://localhost:5000/api/mri/patients/${patientId}/sequences`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const fetchCurrentImage = async () => {
  if (!predictionForm.value.patientId || !predictionForm.value.sequenceId)
    return;
...
const predict = async () => {
  ...
  try {
    ...
    // 始终使用RGB图像的item_id
    const rgbImageIds = JSON.parse(
      localStorage.getItem("rgbImageItemIds") || "[]"
    );
    ...
    console.log("预测数据详情:", {
      cropPosition: cropPosition.value,
      originalPoints: needlePoints.value.map((p) => ({
        x: p.cropX,
        y: p.cropY,
      })),
    ...
    if (response.data.success) {
      ...
      // 更新预测结果
      const resultPath = response.data.prediction.result_path.replace(
        /\\/g,
        "/"
      );
    ...
    ElMessage.error(
      "预测失败: " + (error.response?.data?.message || error.message)
    );
  } finally {
...
onMounted(() => {
  ...
  if (needlingArea.value) {
    needlingArea.value.addEventListener("contextmenu", (e) =>
      e.preventDefault()
    );
  }
});
