      ...
      <div v-loading="loading">
        <template v-if="sequenceInfo">
          <div class="sequence-meta">
            <el-descriptions :column="2" border>
              ...
              <el-descriptions-item label="患者姓名">{{
                sequenceInfo.patient_name
              }}</el-descriptions-item>
              <el-descriptions-item label="图像数量"
                >{{ sequenceInfo.items?.length || 0 }} 组</el-descriptions-item
              >
            </el-descriptions>
          </div>
          ...
          <div class="image-list">
            <el-table
              :data="sequenceInfo.items"
              style="width: 100%"
              v-if="sequenceInfo.items?.length"
            >
              <el-table-column label="序号" type="index" width="80" />
              <el-table-column label="RGB图像" width="300">
                <template #default="scope">
                  <el-image
                    :src="scope.row.rgb.path"
                    :preview-src-list="[scope.row.rgb.path]"
                    fit="contain"
                    class="preview-image"
                  >
                    <template #error>
...
const fetchSequenceDetail = async () => {
  try {
    ...
    console.log("正在获取序列详情:", {
      ...
      }/api/mri/patients/${patientId}/sequences/${sequenceId}`,
    });
