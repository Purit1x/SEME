...
const handleSearch = () => {
  if (!searchQuery.value) {
    ...
    filteredPatientList.value = patientList.value.filter(
      (patient) =>
  ...
  await patientFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        ElMessage.error(
          "添加患者失败：" + error.response?.data?.message || error.message
        );
      }
    }
  });
...
const handleDelete = (row) => {
  ElMessageBox.confirm("确定要删除该患者吗？", "警告", {
